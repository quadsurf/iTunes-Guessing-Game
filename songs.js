numCorrect = 0;
numIncorrect = 0;
$(function() {
var userGuess, albumName, artistName, trackName;

$("#num-correct").append(numCorrect);
$("#num-incorrect").append(numIncorrect);

 var songIds = ["995535015", "966411602", "823593456", "956689796", "943946671",
                 "982388023", "907242704", "201281527", "656801339", "910038357",
                 "250038575", "878000348",  "794095205",  "1645339",  "400835962",
                 "325618",  "169003415",  "51958108",
                 "192688540", "684811768", "344799464", "217633921",
                 "192811017", "71068886", "640047583", "517438248",
                "656479859", "310237", "991390352", "901614155",
                 "344799727", "162337613", "121695005", "159293848", "305118379" ];




   //console.log(songIds);

   function randomSong() {
     var randomIndex = Math.floor(Math.random() * songIds.length);
     return songIds[randomIndex];
   }



   $.ajax({
     url: "https://itunes.apple.com/us/lookup?id=" + randomSong(),
     jsonp: "callback",
     dataType: "jsonp"
   }).done(function(data) {

     var audioSource = data.results[0].previewUrl;

     $('#audio_preview').attr('src', audioSource);

     albumName = data.results[0].collectionName;
     artistName = data.results[0].artistName;
     trackName = data.results[0].trackName;

   })

    $('form').on('submit', function (e) {
     e.preventDefault();

     userGuess = $('input:text[name=userGuess]').val();


     if (userGuess === artistName || userGuess === trackName) {
      //  localStorage.setItem("numCorrect", 0);
      //  numCorrect ++;
       $("#num-correct").append(numCorrect++);
       console.log("Corrects: ", numCorrect);

     } else {
      //  numIncorrect ++;
       $("#num-incorrect").append(numIncorrect++);
       console.log("Incorrects: ", numIncorrect);
     }

     $("#user-guess-display").append(userGuess);


   })




});
